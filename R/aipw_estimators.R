...
#' Construct nuisance-model jobs for the Fit+CV reuse estimator.
#'
#' Builds the four HAL tasks needed by the local demo: two proposed Q
...
#'
#' @param dat Combined Simulation 2 data frame.
...
make_fitcvreuse_jobs <- function(dat, seed, n_trial, n_aux, config,
                                 cache_dir = NULL) {
  ...
  make_fit_job <- function(label, X, Y, weights = NULL) {
    ...
    # dictionary than Brian's c(25, 10) setting and is tracked in cache and
    # checkpoint identities.
    num_knots <- SIM2_DEFAULT_NUM_KNOTS
...
#' Prepare one Simulation 2 run for HAL fitting.
#'
...
prepare_sim2_fitcvreuse_local <- function(m, n_trial, n_aux, p_resp, p_cens,
                                          seed, config, cache_dir = NULL) {
  sim <- simulate_sim2_data(m, n_trial, n_aux, p_resp, p_cens, seed)
  job_bundle <- make_fitcvreuse_jobs(
    dat = sim$dat,
    seed = seed,
    n_trial = sim$n_trial,
    n_aux = sim$n_aux,
...
#' Assemble one Simulation 2 result after all four HAL fits finish.
#'
#' @param prepared Output from `prepare_sim2_fitcvreuse_local()`.
#' @param fit_outputs Named outputs from `run_one_hal_job()`.
#'
#' @return A list with `results` and `fit_timings` data frames.
assemble_sim2_fitcvreuse_local <- function(prepared, fit_outputs) {
  if (!is.list(prepared) || !is.list(prepared$analysis)) {
    stop("prepared must come from prepare_sim2_fitcvreuse_local().", call. = FALSE)
  }
...
#' Run one complete Simulation 2 replicate with the legacy per-run dispatcher.
#'
...
#'
#' @return A list with `results` and `fit_timings` data frames.
sim2_fun_fitcvreuse_local <- function(m, n_trial, n_aux, p_resp, p_cens, seed,
                                      config, cache_dir = NULL,
                                      use_cache = TRUE, fit_cluster = NULL,
                                      cv_cluster = NULL) {
  prepared <- prepare_sim2_fitcvreuse_local(
    m = m,
    n_trial = n_trial,
    n_aux = n_aux,
    p_resp = p_resp,
  ...
  fit_outputs <- run_hal_jobs_local(
    prepared$jobs,
  ...
  prepared$jobs <- NULL
  assemble_sim2_fitcvreuse_local(prepared, fit_outputs)
}
