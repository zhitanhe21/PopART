###############################################################################
# Cross-run global HAL-fit queue for local computers
###############################################################################
...
global_hal_default_task <- function(spec, use_cache) {
  ...
  if (!is.function(run_job) && isNamespaceLoaded("aipwLocalDemo")) {
    run_job <- get("run_one_hal_job", envir = asNamespace("aipwLocalDemo"))
  }
    ...
    if (isTRUE(source_mode)) {
      ...
      stop(
        paste0(
          ...
          "containing R/source_all.R or an installed aipwLocalDemo package."
        ),
        call. = FALSE
      )
    }
      ...
      try(
        slot$run(function() {
          ...
          if (!is.function(shutdown_worker) &&
              isNamespaceLoaded("aipwLocalDemo")) {
            shutdown_worker <- get(
              "shutdown_global_hal_worker",
              envir = asNamespace("aipwLocalDemo"),
              inherits = FALSE
            )
          }
          if (is.function(shutdown_worker)) {
...
#' @param project_dir Optional local demo source directory used to initialize
#'   HAL slots. When omitted outside source mode, workers load the installed
#'   `aipwLocalDemo` namespace.
