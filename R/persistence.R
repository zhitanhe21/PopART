...
FIT_CACHE_SCHEMA <- "fitcvreuse_fit_cache_v2"
RUN_CHECKPOINT_SCHEMA <- "fitcvreuse_run_checkpoint_v1"
LOCAL_DEMO_IMPLEMENTATION <- "fitcvreuse_local_v2"
LOCAL_DEMO_CODE_SCHEMA <- "aipwLocalDemo_algorithm_20260805_v1"
...
#' Create a fingerprint for the computational local-demo source code.
#'
#' @param project_dir Character path to the demo directory.
#'
...
make_code_fingerprint <- function(project_dir = NULL) {
  if (is.null(project_dir) || !nzchar(project_dir)) {
    project_dir <- system.file(package = "aipwLocalDemo")
  }
  if (!nzchar(project_dir)) {
    return(hash_r_object(list(
      schema = LOCAL_DEMO_CODE_SCHEMA,
      package_version = package_version_string("aipwLocalDemo")
    )))
  }

  project_dir <- normalizePath(project_dir)
  relative <- c(
    file.path("R", "setup_helpers.R"),
    file.path("R", "persistence.R"),
    file.path("R", "sim2_data.R"),
    file.path("R", "model_fitting.R"),
    file.path("R", "global_scheduler.R"),
    file.path("R", "aipw_estimators.R"),
    file.path("R", "sim2_demo.R")
  )
