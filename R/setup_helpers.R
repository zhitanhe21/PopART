###############################################################################
# Setup, CLI parsing, and local parallel configuration helpers
###############################################################################
...
SIM2_DEFAULT_CV_FOLDS <- 3L
SIM2_DEFAULT_NUM_KNOTS <- c(5, 3)


#' Load packages required by the local demo.
#'
#' This helper centralizes package loading for the demo scripts and functions.
...
load_fitcvreuse_packages <- function() {
  packages <- c(
  ...
  package_mode <- isNamespace(environment(load_fitcvreuse_packages))
...
#'
#' The demo uses explicit R-level parallelism for HAL fits and cross-validation.
...
#' Create the Simulation 2 sample-size grid.
#'
#' @param scale Character scale name. `demo` is intended for local checks,
#'   `small` reproduces the small Simulation 2 grid, and `large` gives the
#'   large-sample grid.
#'
#' @return A data frame with columns `n_trial` and `n_aux`.
#' @export
fitcv_sample_grid <- function(scale = "demo") {
  ...
  sample_values <- switch(
    ...
    demo = c(120L, 300L),
    small = c(500L, 5000L),
    large = c(100000L, 500000L)
  )
  expand.grid(n_trial = sample_values, n_aux = sample_values)
}


#' Expand sample-size settings into a Brian-compatible Monte Carlo grid.
#'
#' Each Monte Carlo replicate uses one seed across all requested sample-size
#' combinations, matching the `expand.grid(..., sim.id=...)` organization in
#' the original Simulation 2 script.
#'
...
#'   `mc_reps`, and `seed`.
#' @export
expand_sim2_mc_grid <- function(sizes, mc_reps = 1L,
                                base_seed = 91000000L) {
...
print_fitcv_plan <- function(config) {
  cat("Local global-HAL scheduler plan\n")
