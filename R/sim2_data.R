###############################################################################
# Simulation 2 data generation
###############################################################################


#' Simulate clustered Population ART Simulation 2 data.
#'
#' Generates one trial sample and one auxiliary sample using the same
#' data-generating mechanism as the optimized Simulation 2 demo. Trial and
...
#' @param n_aux Integer nominal auxiliary sample size.
#' @param p_resp Numeric response probability setting. Simulation 2 uses the
#'   original fixed value `0.5`; changing it would require deriving a new
#'   response-model intercept.
#' @param p_cens Numeric censoring probability setting. Simulation 2 uses the
#'   original fixed value `0.3`; changing it would require deriving a new
#'   censoring-model intercept.
#' @param seed Integer random seed for the full simulation replicate.
#'
#' @return A list with:
#' \itemize{
#'   \item `dat`: combined trial/auxiliary data frame.
#'   \item `eta0`, `eta1`: true potential outcome means in the trial sample.
#'   \item `n_trial`, `n_aux`: rounded sample sizes actually used.
#' }
#' @export
simulate_sim2_data <- function(m, n_trial, n_aux, p_resp, p_cens, seed) {
