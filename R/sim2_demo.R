...
# Main local optimized PopART Simulation 2 workflow
...
#' @noRd
refresh_resumed_metadata <- function(data, run_id, scale, run_index,
  ...
  source_fields <- c(
    "run_id", "scale", "total_runs", "mc_rep", "mc_reps",
  ...
  current <- list(
    run_id = run_id,
    scale = scale,
    implementation = LOCAL_DEMO_IMPLEMENTATION,
    cpu_plan = cpu_plan,
...
#' Run the version-2 local optimized Simulation 2 AIPW/HAL demo.
#'
#' This entry point uses a bounded cross-run global HAL queue, persistent local
#' worker slots, content-addressed fit caching, successful-run resume
#' checkpoints, and per-run atomic CSV shards.
#'
#' @param project_dir Optional character path to a source checkout. Installed
#'   package users can leave this as `NULL`; it is retained for compatibility
#'   with the repository CLI and source-based workflows.
#' @param scale Character sample grid name: `demo`, `small`, or `large`.
...
#'   original Brian Simulation 2 grid organization.
...
#'   preserved Simulation 2 DGP.
#' @param p_cens Numeric censoring probability setting, fixed at `0.3` by the
#'   preserved Simulation 2 DGP.
#' @param n_trial,n_aux Optional integers for running one explicit sample-size
#'   pair instead of a named grid.
...
#' @param write_sd_file Logical, whether to also write `sd<cluster_id>.csv`.
#' @param cluster_id Integer identifier used for the optional Brian-style `sd`
#'   output file.
#'
...
#' plan <- run_sim2_demo(
#'   run_id = "demo_plan",
#'   out_dir = file.path(tempdir(), "aipw-demo-plan"),
#'   cache_dir = file.path(tempdir(), "aipw-demo-cache"),
#'   use_cache = FALSE,
#'   resume = FALSE,
#'   max_runs = 1,
#'   total_cores = 2,
#'   dry_run = TRUE
#' )
run_sim2_demo <- function(
    project_dir = NULL,
    scale = "demo",
    run_id = paste0("local_", format(Sys.time(), "%Y%m%d_%H%M%S")),
    base_seed = 91000000L,
    out_dir = file.path(getwd(), "aipw-output"),
    cache_dir = file.path(
      tools::R_user_dir("aipwLocalDemo", which = "cache"),
      "hal"
    ),
    use_cache = TRUE,
    resume = TRUE,
  ...
  if (is.null(project_dir) || !nzchar(project_dir)) {
    installed_root <- system.file(package = "aipwLocalDemo")
    project_dir <- if (nzchar(installed_root)) installed_root else getwd()
  }
  ...
  if (length(p_resp) != 1L || !is.finite(p_resp) ||
    ...
    stop(
      "The preserved Simulation 2 DGP requires p_resp = 0.5.",
      call. = FALSE
    )
  }
  if (length(p_cens) != 1L || !is.finite(p_cens) ||
    ...
    stop(
      "The preserved Simulation 2 DGP requires p_cens = 0.3.",
      call. = FALSE
    )
  }
  hal_env_names <- c(
  ...
  sizes <- fitcv_sample_grid(scale)
  if (!is.null(n_trial) || !is.null(n_aux)) {
    if (is.null(n_trial) || is.null(n_aux)) {
      stop("Use n_trial and n_aux together for a single combo.", call. = FALSE)
    }
    sizes <- data.frame(
  ...
  config <- configure_fitcv_parallel(
    n_fit_jobs = 4L,
    default_cv_folds = SIM2_DEFAULT_CV_FOLDS,
  ...
  prefix <- paste("sim2", scale, "fitcvreuse_local", run_id, sep = "_")
  ...
  sd_file <- file.path(out_dir, paste0("sd", as.integer(cluster_id), ".csv"))
  ...
  plan_output$implementation <- LOCAL_DEMO_IMPLEMENTATION
  ...
  cat("Running local optimized Simulation 2 AIPW/HAL demo v2\n")
  cat("  scale = ", scale, "\n", sep = "")
  cat("  run_id = ", run_id, "\n", sep = "")
  ...
  if (isTRUE(write_sd_file)) {
    cat("  Brian-style sd file = ", sd_file, "\n", sep = "")
  }
  print_fitcv_plan(config)
  flush.console()
  ...
  output_files <- list(
    ...
    sd = if (isTRUE(write_sd_file)) sd_file else NULL
  )
  ...
  if (identical(config$scheduler, "global_hal_queue")) {
    ...
    finalize_global_run <- function(ii, context, fit_outputs, error, metrics) {
      ...
      if (identical(status, "ok")) {
        assembled <- tryCatch(
          assemble_sim2_fitcvreuse_local(context, fit_outputs),
          error = function(e) e
        )
  ...
  if (nrow(result_output) > 0L) {
    ...
    if (isTRUE(write_sd_file)) {
      atomic_write_csv(result_output, sd_file, overwrite = TRUE)
    }
  } else {
    if (file.exists(result_file)) {
      quarantine_invalid_file(result_file, "no-current-results")
    }
    if (isTRUE(write_sd_file) && file.exists(sd_file)) {
      quarantine_invalid_file(sd_file, "no-current-results")
    }
  }
  ...
  cat("Finished local Simulation 2 fit+CV reuse outcome v2\n")
  ...
  if (isTRUE(write_sd_file) && nrow(result_output) > 0L) {
    cat("Wrote Brian-style results to: ", sd_file, "\n", sep = "")
  }
