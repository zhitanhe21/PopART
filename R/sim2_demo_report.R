...
# Reporting helpers for the local optimized PopART Simulation 2 workflow

###############################################################################
###############################################################################


#' Write a lightweight Markdown report for a local demo run.
#'
#' Reads the timing, fit timing, and estimator result CSVs written by
#' `run_sim2_demo()` and writes a compact Markdown report containing runtime,
#' fit timing, and estimate summaries.
#'
#' @param project_dir Optional source-checkout path retained for compatibility.
#'   Installed package users can leave it as `NULL`.
#' @param scale Character sample grid name used by the run.
...
#' @export
write_sim2_demo_report <- function(project_dir = NULL,
                                   scale = "demo",
                                   run_id,
  ...
  report <- c(
    "# Local Optimized AIPW/HAL Demo v2 Report",
    "",
...
#' Summarize Simulation 2 estimates for plotting.
#'
#' Converts wide estimator output into long-form truth, estimate, and variance
#' summaries for eta(0), eta(1), RD, and RR.
#'
#' @param results Data frame returned by `run_sim2_demo()` or read from the
...
#' @export
prepare_sim2_summary_table <- function(results) {
  results %>%
...
#' Write the three standard Simulation 2 figures.
#'
...
#' @export
write_sim2_demo_figures <- function(project_dir = NULL,
                                    scale = "demo",
                                    run_id,
                                    out_dir = file.path(getwd(), "aipw-output"),
  ...
  summary_table <- prepare_sim2_summary_table(sim_res)
