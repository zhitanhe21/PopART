...
| `fit_diagnostics`                           | Outer-fold labels, HAL fit sizes, timings, selected lambdas, and worker process IDs |
| `diagnostics`                               | Sample and outer-fold sizes, prediction ranges, weight scaling, and messages        |
| `nuisance_fits`                             | HAL objects grouped by outer fold when explicitly retained; otherwise `NULL`        |
...
cross-fitting, one analysis queues 12 complete HAL fits (four
nuisance-model types in each fold). Each fit still uses the separate
three-fold internal CV to choose its lasso penalty.
